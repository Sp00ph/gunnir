...
fn rook_mask(sq: Square) -> Bitboard {
    let rank1_inner = Rank::R1
        .bitboard()
        .subtract(File::A.bitboard() | File::H.bitboard());
    let file1_inner = File::A
        .bitboard()
        .subtract(Rank::R1.bitboard() | Rank::R8.bitboard());

    (rank1_inner.shift::<Up>(sq.rank().idx() as i8)
        | file1_inner.shift::<Right>(sq.file().idx() as i8))
    .subtract(sq.bitboard())
}
...
mod magic {
    ...
    pub fn generate(w: &mut impl Write) -> io::Result<()> {
        ...
        writeln!(
            w,
            "#[rustfmt::skip]\nstatic ROOK_MAGICS: CachePadded<[Magic; 64]> = CachePadded(["
        )?;
        ...
        writeln!(
            w,
            "#[rustfmt::skip]\nstatic BISHOP_MAGICS: CachePadded<[Magic; 64]> = CachePadded(["
        )?;
...
mod bmi2 {
    ...
    pub fn generate(w: &mut impl Write) -> io::Result<()> {
        ...
        writeln!(
            w,
            "#[rustfmt::skip]\nstatic ROOK_MAGICS: CachePadded<[Magic; 64]> = unsafe {{ CachePadded(["
        )?;
        ...
        writeln!(
            w,
            "#[rustfmt::skip]\nstatic BISHOP_MAGICS: CachePadded<[Magic; 64]> = unsafe {{ CachePadded(["
        )?;
